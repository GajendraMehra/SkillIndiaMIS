...
<div class="trans-detail-view">
    ...
    <div class="card card-primary border-info">
           ...
           <?= Html::a(Yii::t('app', '<i class="fa fa-print" aria-hidden="true"></i>&nbsp;Print'), 'javascript:void(0)', ['class' => 'btn btn-primary','onclick'=>'PrintElem()']) ?>
           <?= Html::a(Yii::t('app', '<i class="fa fa-print" aria-hidden="true"></i>&nbsp;Decelaration'), ['decelare','id'=>$model->id], ['class' => 'btn btn-info',]) ?>
            ...
            <h5 class="mb-2 text-uppercase">Batch Detail   <p class="text-right font-weight-normal pull-right"> Invoice No. - <?= Yii::$app->config->get('invoice_pre','UK/IN/0/00') .$model->id; ?></p></h5>
                ...
                'attributes' => [
                    ...
                    [
                        ...
                        'format'=>'html',
                        'value'=>function($model){
                            return   Html::a( $model->tc->parenttp->tp_name, ['tpartner/accview','id'=> $model->tc->parenttp->id], ['target'=>'_blank','class'=>'text-primary']);
...
<?php if ($model->status==1): ?>
    ...
    <div class="col-md-6 mx-auto text-center">
       ...
       <?= '<label class="control-label has-star">Take Action </label>'; ?>
        <?= '<div class="form-group">'; ?>
        <?= $form->field($model, 'status')->textInput()->widget(Select2::classname(), [
                    'data' => ['3'=>"Approve","Decline"],
                    'options' => ['placeholder' => 'Select ...'],
                    'pluginOptions' => [
                        'allowClear' => true
                    ],
                ])->label(false) ?>   
            <?= $form->field($model, 'is_tds_deduct')->textInput()->widget(Select2::classname(), [
                    'data' => ['0'=>"No","Yes"],
                    'options' => ['placeholder' => 'Select ...'],
                    'pluginOptions' => [
                        'allowClear' => true
                ...
                <?= $form->field($model, 'message_accountant')->textArea() ?>
                 <?= $form->field($model, 'net_amount')->hiddenInput([])->label(false); ?>
