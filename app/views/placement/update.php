<?php

use yii\helpers\Html;

/* @var $this yii\web\View */
/* @var $model app\models\StudentPlacement */

$this->title = 'Update Student Placement: ' . $model->batch->batch_name;
$this->params['breadcrumbs'][] = ['label' => 'Student Placements', 'url' => ['index']];
$this->params['breadcrumbs'][] = ['label' => $model->batch->batch_name, 'url' => ['view', 'id' => $model->id]];
$this->params['breadcrumbs'][] = 'Update';
?>
<div class="student-placement-update">
<div class="card card-primary border-info">
        <div class="card-header text-white bg-<?= Yii::$app->config->get('panel-theme','primary')?>">
        <h5><?= $this->title ?></h5>
        </div>
        <div class="card-body">
        <?= $this->render('_form', [
            'model' => $model,

        ]) ?>
        </div>
    </div>

</div>
